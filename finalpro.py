...
def attfpage():
    ...
    # Create the show_text widget first
    ...
    def recog():
        ...
        try:
            with sr.Microphone() as source:
                ...
                showinfo("python says", "Recognition Done! Processing speech...")
            
            try:
                text = r.recognize_google(audio)
                print(f"Recognized text: {text}")
                # Write recognized text to demo.txt
                with open("demo.txt", 'w') as txtfile1:
                    txtfile1.write(text)
                ...
                showinfo("python says", "Sorry, couldn't understand what you said. Please try again.")
            except sr.RequestError as e:
                print(f"Could not request results from Google Speech Recognition service; {e}")
                showinfo("python says", f"Network error: {e}. Check your internet connection.")
        except Exception as e:
            print(f'Error during recording: {e}')
            showinfo("python says", f"Error with microphone: {e}")
    
    def save_file():
        # Dialog Box to open Text File.
        savefile = filedialog.asksaveasfilename(initialdir='This PC', title = 'Save File', 
                                               filetypes= (('Text Files','txt.*'), ('All Files','*.*')))
        if savefile:  # Check if user didn't cancel
            # Open Saved File
    ...
    # Now create the buttons after all functions are defined
...
def attf1():
    ...
    def printit1():
        ...
        def selectf():global filelocation, text
    filelocation = askopenfilename()
    
    if not filelocation:  # User canceled file selection
        return
    
    r = sr.Recognizer()
    try:
        with sr.AudioFile(filelocation) as source:
            # Adjust for potential noise in the audio file
            r.adjust_for_ambient_noise(source)
            audio = r.record(source)  # Record the entire file
        ...
        try:
            ...
            # Write recognized text to demo1.txt
            with open("demo1.txt", 'w') as txtfile1:
                txtfile1.write(text)
            ...
            print("Google Speech Recognition could not understand audio in file")
            showinfo("python says", "Couldn't recognize speech in the audio file. Try a clearer recording.")
        except sr.RequestError as e:
            print(f"Could not request results from Google Speech Recognition service; {e}")
            showinfo("python says", f"Network error: {e}. Check your internet connection.")
    except Exception as e:
        print(f'Error processing audio file: {e}')
        showinfo("python says", f"Error with audio file: {e}. Make sure it's a valid audio format.")
    def save_file1():
